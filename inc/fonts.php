...
if ( ! function_exists( 'viewpoint_default_fonts' ) ) :
/**
 * Register Google fonts
 */
    function viewpoint_default_fonts() {
        $fonts_url = '';
        $fonts     = array();
        ...
        if ( 'off' !== _x( 'on', 'Lora font: on or off', 'viewpoint' ) ) {
        ...
        if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'viewpoint' ) ) {
            $fonts[] = 'Open+Sans:300italic,400italic,600italic,700italic,800italic,400,300,600,700,800';
        }
        /* translators: To add an additional character subset specific to your language, translate this to 'greek', 'cyrillic', 'devanagari' or 'vietnamese'. Do not translate into your own language. */
        $subset = _x( 'no-subset', 'Add new subset (greek, cyrillic, devanagari, vietnamese)', 'viewpoint' );
        if ( 'cyrillic' == $subset ) {
...
if ( ! function_exists( 'viewpoint_add_editor_font_styles' ) ) :
  /**
   * Register Google fonts for the post editor
   */
  function viewpoint_add_editor_font_styles() {
  ...
  add_action( 'after_setup_theme', 'viewpoint_add_editor_font_styles' );
